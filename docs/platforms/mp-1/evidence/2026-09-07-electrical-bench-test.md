...
| Servo rail voltage, three servos connected idle | Stable near BEC nominal | 5.24 V | PASS | Three EMAX servos connected on MAIN OUT 1, 2, and 4; measured on unused MAIN OUT 5 with battery power applied |
...
| MAIN OUT 8 throttle-signal check | Valid minimum-throttle PWM output | Test servo moved to endpoint and held when connected to MAIN OUT 8 with Output 8 set to Throttle (~1100 µs) | PASS | Confirms Pixhawk MAIN OUT 8 signal path is active and producing a valid low-throttle PWM command |
| ESC initialization from MAIN OUT 8 | ESC recognizes throttle signal and 4S battery; motor remains stopped at minimum throttle | Motor emitted 1-2-3 startup sequence, four cell-count beeps, then long ready tone | PASS | Confirms Skywalker ESC receives MAIN OUT 8 throttle signal, recognizes 4S pack, and reaches ready state at minimum throttle with propeller removed |
...
- After reboot, `Compass not calibrated` no longer appeared in the pre-arm messages. Later testing set `BRD_SAFETY_DEFLT = 0` because MP-1 is not currently using a physical safety switch. After reboot and a clean compass/GPS initialization with the Pixhawk and M10 aligned on the bench, both `Hardware safety switch` and `Compasses inconsistent` warnings cleared. The current pre-arm state is reduced to `Waiting for RC` only.
...
- With the Skywalker ESC three-wire control/BEC lead connected to MAIN OUT 8 and one EMAX ES3059MD servo connected on MAIN OUT 1, the shared MAIN OUT servo rail measured 5.24 V at an unused output connector while the aircraft electronics were powered from the flight battery. This confirms the ESC BEC is successfully energizing the Pixhawk servo rail with essentially no voltage drop at idle.
- With all three planned EMAX ES3059MD servos connected simultaneously on MAIN OUT 1, 2, and 4, the shared MAIN OUT rail remained at 5.24 V when measured at MAIN OUT 5. This confirms the BEC maintains nominal rail voltage with all three servos connected at idle; a simultaneous-motion dynamic load test remains pending.
...
- Mission Planner Battery Capacity was corrected from 3300 mAh to 5000 mAh to match the selected Spektrum SPMX54S50H5 flight battery.
- For ESC signal-path verification, Output 8 was temporarily configured as Throttle. A test servo connected to MAIN OUT 8 moved to an endpoint and held at the low-throttle command, confirming valid PWM output from the Pixhawk. The Skywalker ESC was then restored to MAIN OUT 8 and the F90 motor reconnected with the propeller removed. On power-up, the motor emitted the expected startup sequence, four cell-count beeps for the 4S battery, and a final long ready tone. This confirms the ESC is receiving the MAIN OUT 8 throttle signal and reaching its ready state at minimum throttle.
